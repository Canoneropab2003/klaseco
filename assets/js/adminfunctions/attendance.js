...
(function () {
  ...
  function exportAttendanceCSV(attendanceSection, btn) {
    ...
    // Show loader while SheetJS loads + processes
    const overlay = showPdfLoader(btn, "Generating CSV…");
  ...
  // ── PDF Loading Overlay ──────────────────────────────────
  function showPdfLoader(btn, label) {
    label = label || "Generating PDF…";
    // Inject keyframe styles once
    if (!document.getElementById("att-pdf-loader-style")) {
      const style = document.createElement("style");
      style.id = "att-pdf-loader-style";
      style.textContent = `
        @keyframes att-spin { to { transform: rotate(360deg); } }
        @keyframes att-fade-in { from { opacity: 0; transform: scale(0.94); } to { opacity: 1; transform: scale(1); } }
        @keyframes att-pulse-dot { 0%,80%,100% { transform: scale(0.6); opacity: 0.4; } 40% { transform: scale(1); opacity: 1; } }

        #att-pdf-overlay {
          position: fixed; inset: 0; z-index: 999999;
          display: flex; align-items: center; justify-content: center;
          background: rgba(13, 43, 52, 0.55);
          backdrop-filter: blur(4px);
          -webkit-backdrop-filter: blur(4px);
          animation: att-fade-in 0.22s ease;
        }
        #att-pdf-overlay .att-pdf-card {
          background: #fff;
          border-radius: 20px;
          padding: 32px 40px;
          display: flex; flex-direction: column; align-items: center; gap: 18px;
          box-shadow: 0 24px 60px rgba(0,0,0,0.22), 0 0 0 1px rgba(13,43,52,0.06);
          min-width: 220px;
          animation: att-fade-in 0.28s cubic-bezier(.34,1.56,.64,1);
        }
        #att-pdf-overlay .att-pdf-spinner {
          width: 48px; height: 48px; border-radius: 50%;
          border: 4px solid rgba(13,43,52,0.1);
          border-top-color: #B8F86F;
          animation: att-spin 0.75s linear infinite;
        }
        #att-pdf-overlay .att-pdf-label {
          font-family: system-ui, sans-serif;
          font-weight: 800; font-size: 14px;
          color: #0d2b34; letter-spacing: 0.2px;
        }
        #att-pdf-overlay .att-pdf-dots {
          display: flex; gap: 6px; align-items: center;
        }
        #att-pdf-overlay .att-pdf-dots span {
          width: 7px; height: 7px; border-radius: 50%;
          background: #B8F86F;
          animation: att-pulse-dot 1.2s ease-in-out infinite;
        }
        #att-pdf-overlay .att-pdf-dots span:nth-child(2) { animation-delay: 0.2s; }
        #att-pdf-overlay .att-pdf-dots span:nth-child(3) { animation-delay: 0.4s; }
      `;
    ...
    if (btn) {
      ...
      btn.innerHTML = `<svg width="15" height="15" viewBox="0 0 15 15" style="animation:att-spin .75s linear infinite;flex-shrink:0" fill="none"><circle cx="7.5" cy="7.5" r="6" stroke="currentColor" stroke-width="2.5" stroke-dasharray="28" stroke-dashoffset="10"/></svg> Generating…`;
      btn.style.opacity = "0.8";
    }

    // Overlay
    const overlay = document.createElement("div");
    overlay.id = "att-pdf-overlay";
    overlay.innerHTML = `
      <div class="att-pdf-card">
        <div class="att-pdf-spinner"></div>
        <div class="att-pdf-label">${label}</div>
        <div class="att-pdf-dots"><span></span><span></span><span></span></div>
      </div>
    `;
  ...
  function hidePdfLoader(overlay, btn) {
  ...
  async function exportAttendancePDF(attendanceSection) {
    ...
    const pdfBtnEl = attendanceSection.querySelector(".btn-pdf");
    const overlay  = showPdfLoader(pdfBtnEl);
    ...
    // ── Build data rows from visible table rows
    const dataRows = visibleRows.map((row) =>
      Array.from(row.querySelectorAll("td")).map((td) => normalizeText(td.textContent))
    );
    ...
    doc.autoTable({
      ...
      columnStyles: {
        ...
        4: { cellWidth: 24, halign: "center" },   // Program
...
`;
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    exportBtn.addEventListener("click", (e) => {
      e.preventDefault();
      exportAttendanceCSV(attendanceSection, exportBtn);
    });
